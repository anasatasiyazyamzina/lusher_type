...
require_once($CFG->dirroot . '/question/type/ddlusher/questionbase.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_ddlusher_question extends qtype_ddtoimgLusher_question_base {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_ddlusher_drag_item {
    ...
    public function summarise() {
        //так как нам не нужен текст, а нужен только номер картинки, то пусть тестом будет все, что угодно
//        if (trim($this->text) != '') {
//            return get_string('summarisechoice', 'qtype_ddlusher', $this);
//        } else {
            return $this->no;
                //get_string('summarisechoiceno', 'qtype_ddlusher', $this->no);
//        }
    }
}
/**
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_ddlusher_drop_zone {
    /** @var int Number of the item */
    public $no;
    ...
    public function summarise() {
        if (trim($this->text) != '') {
            $summariseplace = get_string('summariseplace', 'qtype_ddlusher', $this);
        } else {
            $summariseplace =
                    get_string('summariseplaceno', 'qtype_ddlusher', $this->no);
        }
