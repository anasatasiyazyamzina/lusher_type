...
require_once($CFG->dirroot . '/question/type/ddlusher/questiontypebase.php');

define('QTYPE_ddlusher_BGIMAGE_MAXWIDTH', 600);
define('QTYPE_ddlusher_BGIMAGE_MAXHEIGHT', 400);
define('QTYPE_ddlusher_DRAGIMAGE_MAXWIDTH', 150);
define('QTYPE_ddlusher_DRAGIMAGE_MAXHEIGHT', 100);
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_ddlusher extends qtype_ddtoimgLusher_base {

    protected function make_choice($dragdata) {
        return new qtype_ddlusher_drag_item($dragdata->label, $dragdata->no,
                                        $dragdata->draggroup, $dragdata->infinite, $dragdata->id);
    }

    protected function make_place($dropzonedata) {
        return new qtype_ddlusher_drop_zone($dropzonedata->label, $dropzonedata->no,
                                                    $dropzonedata->group,
    ...
    public function save_question_options($formdata) {
        ...
        $options = $DB->get_record('qtype_ddlusher', array('questionid' => $formdata->id));
        if (!$options) {
            ...
            $options->id = $DB->insert_record('qtype_ddlusher', $options);
        }
        ...
        $DB->update_record('qtype_ddlusher', $options);
        $DB->delete_records('qtype_ddlusher_drops', array('questionid' => $formdata->id));
        foreach (array_keys($formdata->drops) as $dropno) {
            ...
            $DB->insert_record('qtype_ddlusher_drops', $drop);
        }

        // An array of drag no -> drag id.
        $olddragids = $DB->get_records_menu('qtype_ddlusher_drags',
        ...
        foreach (array_keys($formdata->drags) as $dragno) {
            ...
            if ($info['filecount'] > 0 || (trim($formdata->draglabel[$dragno]) != '')) {
                ...
                if (isset($olddragids[$dragno + 1])) {
                    ...
                    $DB->update_record('qtype_ddlusher_drags', $drag);
                } else {
                    $drag->id = $DB->insert_record('qtype_ddlusher_drags', $drag);
                }

                if ($formdata->drags[$dragno]['dragitemtype'] == 'image') {
                    self::constrain_image_size_in_draft_area($draftitemid,
                                        QTYPE_ddlusher_DRAGIMAGE_MAXWIDTH,
                                        QTYPE_ddlusher_DRAGIMAGE_MAXHEIGHT);
                    file_save_draft_area_files($draftitemid, $formdata->context->id,
                                        'qtype_ddlusher', 'dragimage', $drag->id,
                                        array('subdirs' => 0, 'maxbytes' => 0, 'maxfiles' => 1));
                } else {
                    // Delete any existing files for draggable text item type.
                    $fs = get_file_storage();
                    $fs->delete_area_files($formdata->context->id, 'qtype_ddlusher',
                                                                'dragimage', $drag->id);
                }
        ...
        if (!empty($olddragids)) {
            ...
            $DB->delete_records_select('qtype_ddlusher_drags', "id $sql", $params);
        }

        self::constrain_image_size_in_draft_area($formdata->bgimage,
                                                    QTYPE_ddlusher_BGIMAGE_MAXWIDTH,
                                                    QTYPE_ddlusher_BGIMAGE_MAXHEIGHT);
        file_save_draft_area_files($formdata->bgimage, $formdata->context->id,
                                    'qtype_ddlusher', 'bgimage', $formdata->id,
                                    array('subdirs' => 0, 'maxbytes' => 0, 'maxfiles' => 1));
    }
    public function move_files($questionid, $oldcontextid, $newcontextid) {
        ...
        $fs->move_area_files_to_new_context($oldcontextid,
                                    $newcontextid, 'qtype_ddlusher', 'bgimage', $questionid);
        $dragids = $DB->get_records_menu('qtype_ddlusher_drags',
                                                array('questionid' => $questionid), 'id', 'id,1');
        foreach ($dragids as $dragid => $notused) {
            $fs->move_area_files_to_new_context($oldcontextid,
                                    $newcontextid, 'qtype_ddlusher', 'dragimage', $dragid);
        }
    ...
    protected function delete_files($questionid, $contextid) {
        ...
        $dragids = $DB->get_records_menu('qtype_ddlusher_drags',
                                                array('questionid' => $questionid), 'id', 'id,1');
        foreach ($dragids as $dragid => $notused) {
            $fs->delete_area_files($contextid, 'qtype_ddlusher', 'dragimage', $dragid);
        }
    ...
    public function export_to_xml($question, qformat_xml $format, $extra = null) {
        ...
        $output .= $format->write_combined_feedback($question->options,
                                                    $question->id,
                                                    $question->contextid);
        $files = $fs->get_area_files($contextid, 'qtype_ddlusher', 'bgimage', $question->id);
        $output .= "    ".$this->write_files($files, 2)."\n";;

        foreach ($question->options->drags as $drag) {
            $files =
                    $fs->get_area_files($contextid, 'qtype_ddlusher', 'dragimage', $drag->id);
            $output .= "    <drag>\n";
    ...
    public function import_from_xml($data, $question, qformat_xml $format, $extra=null) {
        if (!isset($data['@']['type']) || $data['@']['type'] != 'ddlusher') {
            return false;
        }

        $question = $format->import_headers($data);
        $question->qtype = 'ddlusher';
