YUI.add('moodle-qtype_ddlusher-dd', function(Y, NAME) {
    ...
    var ddlusherDDNAME = 'ddlusher_dd';
    var ddlusher_DD = function() {
        ddlusher_DD.superclass.constructor.apply(this, arguments);
    };

    /**
     * This is the base class for the question rendering and question editing form code.
     */
    Y.extend(ddlusher_DD, Y.Base, {
        doc: null,
        polltimer: null,
                ...
                draggable_for_form: function(drag) {
                    ...
                    dd.on('drag:end', function(e) {
                        ...
                        if (!gooddrop) {
                            ...
                            //this.doc.drag_item(draginstanceno).set('visibility','hidden');
                        }
                    }, this);
        ...
        update_padding_size_for_group: function(groupno) {
            ...
            if (groupitems.size() !== 0) {
                ...
                this.doc.drop_zone_group(groupno).setStyles({
                    'width': maxwidth + 10,
                    'height': maxheight + 10
                });
            }
    ...
    M.qtype_ddlusher = M.qtype_ddlusher || {};
    M.qtype_ddlusher.dd_base_class = ddlusher_DD;

    var ddlusherQUESTIONNAME = 'ddlusher_question';
    var ddlusher_QUESTION = function() {
        ddlusher_QUESTION.superclass.constructor.apply(this, arguments);
    };
    /**
     * This is the code for question rendering.
     */
    Y.extend(ddlusher_QUESTION, M.qtype_ddlusher.dd_base_class, {
        passiveSupported: false,
        pendingid: '',
        initializer: function() {
            this.pendingid = 'qtype_ddlusher-' + Math.random().toString(36).slice(2); // Random string.
        ...
        init_drops: function() {
            ...
            for (var dropno in this.get('drops')) {
                ...
                var nodeclass = 'dropzone place' + dropno;
                var title = drop.text.replace('"', '\"');
                if (!title) {
                    title = M.util.get_string('blank', 'qtype_ddlusher');
                }
    ...
    }, { NAME: ddlusherQUESTIONNAME, ATTRS: {} });
    ...
    M.qtype_ddlusher.init_question = function(config) {
        return new ddlusher_QUESTION(config);
    };
