 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_ddlusher_edit_form extends qtype_ddtoimgLusher_edit_form_base {
    ...
    protected function definition_draggable_items($mform, $itemrepeatsatstart) {
        ...
        $this->repeat_elements($this->draggable_item($mform), $itemrepeatsatstart,
                ...
                get_string('addmoreimages', 'qtype_ddlusher'), true);
    }
    ...
    protected function draggable_item($mform) {
        ...
        $dragitemtypes = array('image' => get_string('draggableimage', 'qtype_ddlusher'),
                                'word' => get_string('draggableword', 'qtype_ddlusher'));
        ...
        $grouparray[] = $mform->createElement('advcheckbox', 'infinite', ' ',
                get_string('infinite', 'qtype_ddlusher'));
    ...
    public function validation($data, $files) {
        ...
        for ($dragindex = 0; $dragindex < $data['noitems']; $dragindex++) {
            ...
            if ($data['drags'][$dragindex]['dragitemtype'] == 'word') {
                ...
                $errormessage = get_string('formerror_disallowedtags', 'qtype_ddlusher', s($allowedtags));
            } else {
                $allowedtags = '';
